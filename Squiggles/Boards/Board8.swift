                        ...
                        ForEach(0..<8) { x in
                            ZStack {
                                ...
                                Circle()
                                    ...
                                    .frame(width: animate && color == dot?.color ? screenWidth/10 : screenWidth/12)
                            }
                        }
                    }
                }
...
extension Board8 {
    private var middle: some View {
        GeometryReader { geo in
            ForEach(viewModel.squiggles, id: \.color) { squiggle in
                ...
                .stroke(squiggle.color, lineWidth: screenWidth/32)
                .offset(CGSize(width: geo.size.width/16, height: (geo.size.height - geo.size.width)/2 +
                               geo.size.width/16))
            }
        }
    }
    ...
    func position(at index: Int) -> CGPoint {
        let row = index / 8
        let col = index % 8
        return CGPoint(x: CGFloat(col) * screenWidth/8, y: CGFloat(row) * screenWidth/8)
    }
}
    ...
    private var drawLine: some View {
        GeometryReader { geo in
            ForEach(viewModel.lines, id: \.color) { line in
                ...
                .stroke(line.color, lineWidth: screenWidth/32)
                .offset(CGSize(width: geo.size.width/16, height: (geo.size.height - geo.size.width)/2 +
                               geo.size.width/16))
            }
        }
                        ...
                        ForEach(0..<8) { w in
                            Rectangle()
                                ...
                                .simultaneousGesture(DragGesture(minimumDistance: 0.0, coordinateSpace: .named("overlay"))
                                    .onChanged { value in
                                        let j = w + h*8
                                        let x = Int(value.location.x/(screenWidth/8))
                                        let y = Int(value.location.y/(screenWidth/8))
                                        var i = x + y*8
                                        if i < 0  { i = 0  }
                                        if i > 63 { i = 63 }
                                        viewModel.start(i: j)
